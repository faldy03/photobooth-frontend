...
import { LayoutDashboard, Ticket, LogOut, Menu, PanelLeftClose, PanelLeftOpen } from "lucide-react";
...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  if (pathname === "/admin/login") {
    return <div className="min-h-screen bg-[#EFE9DB] font-sans">{children}</div>;
  }
      ...
      >
        ...
        <div className="h-20 flex items-center justify-center border-b-[4px] border-retro-charcoal bg-white overflow-hidden whitespace-nowrap">
          {isSidebarOpen ? (
            <h2 className="text-2xl font-black font-serif uppercase tracking-tighter drop-shadow-sm">
              STITCHED<span className="text-retro-red tracking-widest">BOOTH.</span>
            </h2>
          ) : (
        ...
        <nav className="flex-1 py-6 px-3 space-y-4 overflow-y-auto overflow-x-hidden">
          ...
          >
            ...
            {isSidebarOpen && <span className="whitespace-nowrap">Voucher</span>}
          </Link>
        </nav>
      </aside>
          ...
          <div className="ml-auto flex items-center gap-6">
            <div className="flex items-center gap-3 hidden sm:flex">
              <div className="flex flex-col text-right">
                <span className="text-xs font-black uppercase tracking-widest">Admin Utama</span>
                <span className="text-[10px] font-bold text-retro-charcoal/60 uppercase">Manajemen Sistem</span>
              </div>
